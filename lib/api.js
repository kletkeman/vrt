	...
	Route.prototype.prepare = function(key, value) {
		...
		if( dr_key )
		{
			if( vk_dr_key ) 
			{
				if( vk_value)
				{
					payload[vk_value][vindex] = data[vk_dr_key][kindex];
				}
				else
				{
					payload[vk_value][vindex] = this.format[value](data[vk_dr_key][kindex]);
				}
			}
			else 
			{
				if( vk_value )
				{
					payload[vk_value][vindex] = data[dr_key];
				}
				else
				{
