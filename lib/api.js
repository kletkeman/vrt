;!(function(module, require, undefined) {
  ...
  Api.prototype.route = function(widget) {
	...
	for(var path in schema) {
		...
		routes.push(new Api.Route( path, config.bind, $.extend({}, widget.format, config.defaults), widget.write.bind(widget) ));
        ...
        this.store.tree.write(id, data, function(err) {
            ...
            if(err) {
             ...
             context.store.tree.get(id, function(err, tree) {
                ...
                if(root)
                    Api.Route.router(id, context.store.constructor.prototype.Tree.prototype.break.call(tree));
