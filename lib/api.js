             ...
             context.store.tree.get(id, function(err, tree) {
                ...
                for(var path in (data = context.store.constructor.prototype.Tree.prototype.break.call(data, -1)))
                    if(typeof data[path] !== 'object')
                        ...
                        Api.Route.router( 
                            (p = (id + '.' + path)), 
                                (d = context.store.constructor.prototype.Tree.prototype.break.call(  (function(t,p) { for(var i=0;i<p.length;i++) t = t[p[i]]; return t;})(tree, path.split('.'))  )) );
                       
                       context.store.tree.write(p, d);
                         
                    }
                            
                Api.Route.router( id, tree );
