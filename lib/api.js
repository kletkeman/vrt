;!(function(module, require, undefined) {
  ...
  else 
  {
	...
	Route.prototype.remove = function() {
		...
		var routes = this.routes, i = 0, len = routes.length, route;
  ...
  Api.prototype.create = function (fields, callback) {
    ...
    var type = fields.type.capitalize();

    if( typeof vrt.Api[type] === 'undefined' )
      throw new Error('TypeError: Invalid type `'+type+'`');
    
	var obj = new vrt.Api[type](fields);
	
	if(typeof window === 'undefined' && obj.schema) this.route(obj);
    ...
    var context  = this;
  	...
  	if(Guid && !Guid.isGuid(id)) {
        ...
        this.store.tree.write(id, data, function(err) {
            ...
            if(err) {
             ...
             context.store.tree.get(id, function(err, tree) {
                ...
                for(var path in (data = context.store.constructor.prototype.Tree.prototype.break.call(data, -1)))
                    if(typeof data[path] !== 'object') {
                        ...
                        Route.router( 
                            (id + '.' + path), 
                                context.store.constructor.prototype.Tree.prototype.break.call(  (function(t,p) { for(var i=0;i<p.length;i++) t = t[p[i]]; return t;})(tree, path.split('.'))  ));
                            
                if(root)
                    Route.router(id, context.store.constructor.prototype.Tree.prototype.break.call(tree));
	    ...
	    return this.get(id, function(err, dataset) {
		    ...
		    return dataset.write.call(dataset, data, callback);
