        ...
        this.store.tree.write(id, data, function(err) {
            ...
            if(err) {
             ...
             context.store.tree.get(id, function(err, tree) {
                ...
                for(var path in (data = context.store.constructor.prototype.Tree.prototype.break(data, -1)))
                    if(typeof data[path] !== 'object') {
                        ...
                        Route.router( 
                            (id + '.' + path), 
                                context.store.constructor.prototype.Tree.prototype.break(  (function(t,p) { for(var i=0;i<p.length;i++) t = t[p[i]]; return t;})(tree, path.split('.'))  ));
                            
                if(root)
                    Route.router(id, context.store.constructor.prototype.Tree.prototype.break(tree));
