...
MongoStore.prototype._lock = function(name, args) {
    ...
    assert.notEqual(typeof args, 'undefined');
...
MongoStore.prototype._unlock = function(name) {
    
    var lock = this.locks[name] || {length: 0}, args;
    
    assert.equal(typeof name, 'string');
    
    if(lock.length) {
        
        while((args = lock.pop()) && lock.length)
        {
                        
            if(args === MONGOSTORE_LOCK_PUSH_EVENT)
                if(lock.length && lock[lock.length-1] === MONGOSTORE_LOCK_PUSH_EVENT)
                    break;
                else
                    continue;
            
            vrt.ipc.broadcast(MONGOSTORE_LOCK_POP_EVENT, [name]);
...
MongoStore.prototype._construct_set_command = function(mongokey, data) {
    ...
    assert.equal(typeof mongokey, 'string');
		...
		vrt.get(id, function(err, exists) {
			...
			return context.connect(function(err, collection, db, collections) {
				...
				function nextCollection() {
                    ...
                    if(context._lock(name, arguments)) {
                        names.unshift(name);
                        return;
                    }
...
MongoStore.prototype._count = function (collection, callback) {
    ...
    assert.equal(collection instanceof MongoDB.Collection, true);
    assert.equal(typeof callback, 'function');
    
    return collection.stats(function(err, stats) {
...
MongoStore.prototype._insert = function (collection, data) {
   
    try {
    assert.equal(collection instanceof MongoDB.Collection, true);
    assert.equal(typeof data, 'object');
    assert.equal(typeof data._id, 'undefined');
    }
    catch(e){
...
MongoStore.prototype._update = function (collection, data) {
