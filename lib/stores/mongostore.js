...
var MongoStore = function(options) {
    ...
    IPC.define('MONGOSTORE_INSERT_EVENT');
    IPC.define('MONGOSTORE_UPDATE_EVENT');
    ...
    this.on('insert', function(collectionName, data, ipc) {
        ...
        var n, c = context.registers.cache[n = collectionName + '.stats'];
        
        if(c) {
            context._cache_drop(n);
            vrt.log.debug("Dropped cache", n);
        }
    });
...
MongoStore.prototype._cache = function() {
	...
	if(this.registers.cache[name])
		callback.apply(this, this.registers.cache[name]);
	else {
		...
		args.push(function(err) {
			...
			context.registers.cache[name] = arguments;
...
MongoStore.prototype._cache_drop = function(name) {
	delete this.registers.cache[name];
	return this;
};
			...
			return context.connect(function(err, collection, db, collections) {
				...
				function nextCollection() {
					...
					var name 	   = names.shift(),
                        obsolete   = false,
                        onWrite    = function(collectionName, d) {
                            if(obsolete = collectionName === name) {
                                context.removeListener('insert', onWrite);
                                vrt.log.debug("MongoStore#select() Data was inserted into collection \"", collectionName, "\" before current request finished");
                            }
                        };
                    
                    context.on('insert', onWrite);
							...
							if(!row && names.length) {
								...
								collection.find({_id : {$gt : row._id}}, {_id: true})
										  ...
										  .count(function(err, index) {
											  	...
											  	context._cache(collection.collectionName + '.stats', collection.stats, collection, function(err, stats) {
		...
		if (err) {
			...
			return context.connect(function(collections) {
				...
				function mongo_callback(err) {
                    ...
                    if(typeof (info = arguments[2]) === 'object' && info.connectionId && info.updatedExisting)
                        context.emit('update', collection.collectionName, data);
                    else
                        context.emit('insert', collection.collectionName, arguments[1][0]);
                    vrt.ipc.broadcast(MONGOSTORE_INSERT_EVENT, arguments);
				...
				if(exists.bufferSize) {
					...
					if(typeof index === 'number') {
                        ...
                        var obsolete = false,
                            onWrite  = function(collectionName, d) {
                                if((obsolete = collectionName === collection.collectionName)) {
                                    context.removeListener('insert', onWrite);
                                    vrt.log.debug("MongoStore#push() Data was inserted into collection \"", collectionName, "\" before current request finished.");
                                }
                            }
                        
                        context.addListener('insert', onWrite);
						...
						context._cache(collection.collectionName + '.stats', collection.stats, collection, function(err, stats) {
							...
							if (err) {
                                ...
                                return setImmediate(context.push.bind(context, x, y, id, data, callback));
                            }
							...
							cursor.nextObject(function(err, d) {
								...
								if(!d) {
									...
									return collection.insert(data, mongo_callback);
                                }
								else {
                                    vrt.ipc.broadcast(MONGOSTORE_UPDATE_EVENT, [collection.collectionName, data]);
									return collection.update({_id: d._id}, {$set : context._translate_arrays(data)}, mongo_callback);
                                }
                    ...
                    vrt.ipc.broadcast(MONGOSTORE_UPDATE_EVENT, [collection.collectionName, data]);
					return collection.update({'id': id}, context._construct_set_command('data', data), mongo_callback);
                }
