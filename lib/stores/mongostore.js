...
MongoStore.prototype.connect = function() {
	...
	return this._buffer('connect', MongoClient.connect, MongoClient, util.format('mongodb://%s:%d/vrt', this.host, this.port), {server: {poolSize: 50}},
		function(err, db) {
			...
			db.close = function emptyFunction() {};
			...
			db.collection(name, function(err, collection) {
				...
				collection.findOne(selector, function(err, row) {
					...
					if(!row && names.length)
						...
						collection.find({_id : {$lt : row._id}}, {_id: true})
								  .sort({_id : 1})
			...
			function mongo_callback(err) {
				...
				if (err) {
					...
					collection.stats(function(err, stats) {
						...
						else if(!stats.capped && stats.count >= (exists.bufferSize * 1.25)) {
							db.command({convertToCapped: collection.collectionName, size: stats.size }, function(err) {
