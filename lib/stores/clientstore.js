...
ClientStore.prototype.reload = function() {
	...
	(function loadId() { i++;
		...
		new vrt.Api[config.type.substr(0,1).toUpperCase()+config.type.substr(1)](config); 
...
ClientStore.prototype.push = function(x, y, id, data, callback) {
	...
	return $.post("./api/v1/" + id, data, function(response, statustext) {

		if(typeof callback === 'function')
			callback(response.error ? new Error(response.error) : undefined)
		else if(response.error)
			throw new Error(response.error);
