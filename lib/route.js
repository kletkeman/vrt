define(['jquery', 'lib/route.scope', 'guid'], function($, Scope, Guid) {
       ...
       return (function dig(context, d) {
            ...
            for(var k in d) {
                ...
                if( typeof d[k] === 'object' && !Array.isArray(d[k]) )
                    ...
                    result += dispatch(context, stack.join('.'), new Scope.Singleton(k, d) );
                    ...
                    d[k].forEach(function(_,i) {
                        if(typeof d[k][i] === 'object')
                            ...
                            result += dispatch(context, stack.join('.') + '.' + i, new Scope.Singleton(i, d[k]) );
                    });
                }
    ...
    Route.prototype.pack = function(data) {
        ...
        this.payload = $.extend({}, this.format), this.data = data instanceof Scope.Singleton ? data.value : data;
        ...
        for (var name in this.bind) {
            ...
            if (typeof this.bind[name] === 'string') {
                ...
                try {
                    this.prepare(this.bind[name].call( (scope = scope || $.extend(new Scope(), this.obj, {'data': data, 'tree': this.tree, 'params': this.params}) ) ), name);
                }
                catch(e) {
