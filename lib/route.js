define(['lib/route.functions'], function(functions) {
    ...
    Route.router = function(path, data, tree) {
        ...
        for (var i = 0, len = this.routes.length, m = 0, route; i < len; i++)
            if((route = this.routes[i]) && (route.tree = tree) && route._bind)
       ...
       return (function dig(context, d) {
            ...
            for(var k in d) {
                ...
                if( typeof d[k] === 'object' && !Array.isArray(d[k]) )
                    ...
                    result += dispatch(context, stack.join('.'), new functions.Singleton(k, d) );
                    ...
                    d[k].forEach(function(_,i) {
                        if(typeof d[k][i] === 'object')
                            ...
                            result += dispatch(context, stack.join('.') + '.' + i, new functions.Singleton(i, d[k]) );
                    });
                }
    ...
    Route.prototype.prepare = function(key, value) {
        ...
        try {
            ...
            this.obj.onError(new Error([this.obj.id, "an error was thrown probably because of bad schema", this.path, e.message].map(function(s) {return " ( " + s + " ) "; }).join(" ")));
            return false;
        }
    };
    ...
    Route.prototype.pack = function(data) {
        ...
        this.payload = $.extend({}, this.format), this.data = data instanceof functions.Singleton ? data.value : data;
        ...
        for (var name in this.bind) {
            ...
            if (typeof this.bind[name] === 'string') {
                ...
                try {
                    this.prepare(this.bind[name].call( (scope = scope || $.extend(Object.create(functions), this.obj, {'data': data, 'tree': this.tree, 'params': this.params}) ) ), name);
                }
                catch(e) {
                    this.obj.onError(new Error([this.obj.id, "an error was thrown in schema bound function", this.path, name, e.message].map(function(s) {return " ( " + s + " ) "; }).join(" ")));
                    return false;
                }
