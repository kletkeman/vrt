              ...
              if(Array.isArray(that[key]))
                ...
                try {
                    ...
                    if(typeof that[key] === 'undefined')
                        ...
                        debug(["fromJSON()", id, filename, path, "[type of", (that[key] = prev), "cannot be", type, "]", depth].join(' '));
                    else
                        debug(["DataSet#fromJSON()", id, filename, path, "was evaluated [Result:", prev, "=>", that[key], "]", depth].join(' '));
                }
                catch(e) {
                    if(typeof that[key] === 'undefined')
                        throw e;
                    debug(["fromJSON()", id, filename, path, "evaluation error while running #eval()", "[",prev,"]", e, depth].join(' '));
                }
