  ...
  DataSet.prototype.fromJSON = function() {
    ...
    function walk(that) {
        ...
        for(var key in that) {
              ...
              if (depth ===  1 && ['id', 'type'].indexOf(key) > -1)
                continue;
              else if(typeof that[key] === 'object')
                walk(that[key]);
              else if(typeof that[key] === 'string' && that[key].length) {      
                try {
                    that[key] = eval.call({}, (prev = that[key])); path = '"'+stack.join('"."')+'"';
                    if(typeof that[key] === 'undefined')
                        vrt.log.error("DataSet#fromJSON()", id, filename, path, " #eval() returned undefined", e, depth);
                    else if(depth <= 2 && ( (type = typeof that[key]) === 'object' || (type = typeof that[key]) === 'function') )
                        vrt.log.error("DataSet#fromJSON()", id, filename, path, "[type of ", (that[key] = prev), " cannot be ", type, "]", depth);
                    else
                        vrt.log.warn("DataSet#fromJSON()", id, filename, path, " was evaluated [Result: ", prev, " => ", that[key], "]");
                }
                catch(e) { 
                    vrt.log.debug("DataSet#fromJSON()", id, filename, path, " evaluation error while running #eval()", e, depth);
                }
