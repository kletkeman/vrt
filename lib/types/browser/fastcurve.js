...
function(
    ...
    menu.add("delete", "Delete", "Delete this curve.", 
        function show (curveObject, command) {
            command.description = "Delete curve << " + curveObject.label + " >>";
        }
    ),
    ...
    function Curve ( options ) {
        ...
        function peekCorrection () {
            return Math.floor(correction);
        };
        
        this.map.x.peek = peekCorrection;

        Object.defineProperty(this.map.x, 'correction', {
                    
          enumerable: false,
          get: function() {
            var floored = peekCorrection();
            return floored ? ( (correction = correction - floored), floored ) : 0;
          },
          set : function(value) {
            if(value===null) return (correction = 0);
            return (correction += value - Math.floor(value)), Math.floor(correction);
          }
        });
...
*/
        ...
        Curve.prototype.draw = function(data, startTime, endTime, domain) {
                        
            var p0, p1, p2, shift = 0, x = this.map.x, scale = this.scales.x, points;

            context  = this,
            width    = this.width, 
            height   = this.height,
            stack    = this.stack,
            data     = (data || this.map().x.filter(function(d) { return !!d; }));
            ...
            if( Array.isArray(data) && (c = this.canvas.getContext("2d")) ) {
                ...
                if( data.length && data[data.length-1] ) {
                    ...
                    scale.domain(domain || [
                        new Date( startTime || (startTime = (data[data.length-1].timestamp - (this.seconds * 1000))) ),
                        new Date( endTime || (endTime = data[data.length-1].timestamp) )
                    ]);
                ...
                for(var i = 0, len = data.length; data[i] && i < len; i++) {
                    
                    if ( stack.length < 1 ) 
                    {
                        stack.push(data[i]); continue;
                    }
                    ...
                    drawLine.apply(null, (
                        points.map(getCoordinates), 
                        (correction ? points.slice(0, 2).map(correct(-correction, 'x')) : points), points.map(m))), 
                                stack.push(p2), (p1.cpx = p2.cpx), (p1.cpy = p2.cpy);
                ...
                if(arguments.length > 1) {
                    ...
                    p0 = arguments[0];
                    p1 = stack[stack.length - 1];
                    p2 = getCoordinates(arguments[arguments.length-1]);
                    
                    return this.draw.apply(this, 
                        [Array.prototype.slice.call(arguments)].concat(
                            p1 && (shift = p2.x - p1.x) >= 1 ? 
                                ((x.correction = p2.sx), this.shift(-(shift += x.peek())).map(
                                    function (d, i) {                                    
                                        if(i === 0)
                                            return stack.map(correct(-shift, 'cpx')), p0.timestamp;
                                        return d;                                        
                                    })) : []
                        ));
        ...
        function correct (p, key) {
            ...
            else if (typeof p === 'object' && typeof k === 'string' && Number.isFinite(v))
                return (p[k] += v), p;
    ...
    FastCurve.prototype.fromJSON = function () {
        ...
        this.labels && this.labels.forEach(function(label, i) {
            curves[i] = (new Curve($.extend({seconds: context.seconds}, context.defaults, (curves[i] || {label: label}) ) ));
        });
        
        return DataSet.prototype.fromJSON.call(this);    
    };

    FastCurve.prototype.unload = function () {
        return this.stop(), DataSet.prototype.unload.call(this);
    };
    ...
    FastCurve.prototype.create = function () {
        ...
        this.contextmenu.add( 'Release', 'Release and force a re-draw because window is frozen', 
            function click () {
                context.brush.clear(), (extent = null), context.overlay.select("g.brush.x").call(brush), context.render('draw', true);
            },
        ...
        .on("brushend", function() {
            ...
            if(!brush.empty() && (extent = brush.extent())) {
                ...
                context.curves.forEach(function(curveObject, i) { 
                    return curveObject.reset(), curveObject.draw.apply(curveObject, [context.data[i]].concat(brush.extent().map(function(x) { 
    ...
    FastCurve.prototype.update = function () {
        ...
        if( !e ) {}
            ...
            if(!this.curves[e.x] || !this.__animationRequestId__) return;
            
            if(Array.isArray(this.__buffer[e.x]))
                this.__buffer[e.x].push(e.data);
            else if(typeof this.__buffer[e.x] === 'object')
                this.__buffer[e.x] = [this.__buffer[e.x], e.data];
            else
                this.__buffer[e.x] = e.data;           
    ...
    FastCurve.prototype.draw = function ( force ) {
        ...
        function rightclick () {
            ...
            menu.get("delete").click = function (curveObject) {
                return context.delete(context.curves.indexOf(curveObject));
            },
        ...
        return this.curves.forEach(function(curveObject, i) {
            return (!force && !curveObject.dirty) || (curveObject.reset(), curveObject.draw(context.data[i]));
        });
    ...
    FastCurve.prototype.start = function() {
        ...
        return this.render('draw', true), (function dispatch () {

            var data, curve;

            while( context.__buffer.length) {
                
                if( (data  = context.__buffer.pop()) && 
                    (curve = context.curves[context.__buffer.length]) )
                        Array.isArray(data) ? curve.draw.apply(curve, data) : curve.draw(data);
            }
    ...
    function editor ( obj ) {
        ...
        return this.editor({
            ...
            actions: {
                "delete" : function() {
                    return menu.get("delete").click(), this.actions.cancel();
                }
            }
        });
    };
