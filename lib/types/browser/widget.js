...
function (
  ...
  function data_src_dialog (options) {
      ...
      vrt.controls.dialog("data-editor", {"min-width" : "85%", "min-height" : "85%"}, options),
        ...
        layout =
         dialog.insert("titlebar", {text: this.title + " - Data"})
        .insert("layout", {  
      ...
      this.data.options.prepend.call(layout).add(this.data.options);
      
      dialog.on("modified", function (component, name, obj) {
          
          if(name === "url")
              
            context.data.read(function (err) {
                ...
                else if(typeof component.error === "function")
                    component.error(0);
