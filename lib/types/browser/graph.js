...
vrt.Api.Graph.prototype.update = function(data) {

	var index = data.__cursor__,
		height = this.dimensions.height - this.dimensions.margin.top - this.dimensions.margin.bottom,
		x1 = this.x1,
		color = this.color,
		y =  this.y,
		labels = this.labels;
	
	if(d3.max(data.values) > this.max)
		delete this.max;
