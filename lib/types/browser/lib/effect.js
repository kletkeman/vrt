...
function (
   ...
   function default_draw (gl) {
       gl = gl || this._gl;
       return gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4), this;
   }
   ...
   function Effect (fs_src, vs_src, gl, ease, draw) {
    ...
    function texture (texture_, width_, height_) {
        
        var texture = texture_, width = width_, height = height_,
            textures,
            gl      = this._gl,
            canvas  = gl.canvas,
            options = {},
        ...
        for(var p in options) {
            ...
            if ( (i = pixelstorei_enums.indexOf( (p = parseInt(p)) )) > -1 )
                gl.pixelStorei(p, options[p]);
                    
        }
            
        if(image)
            gl.texImage2D(options["target"] || gl.TEXTURE_2D, options["level"] || 0, options["internalFormat"] || gl.RGBA, options["format"] || gl.RGBA, options["type"] || gl.UNSIGNED_BYTE, image);
        else
            gl.texImage2D(options["target"] || gl.TEXTURE_2D, options["level"] || 0, options["internalFormat"] || gl.RGBA, width, height, options["border"] || 0, options["format"] || gl.RGBA, options["type"] || gl.UNSIGNED_BYTE, options["data"] || null);

            gl.texParameteri(options["target"] || gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, options[gl.TEXTURE_MIN_FILTER] || gl.NEAREST);
            gl.texParameteri(options["target"] || gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, options[gl.TEXTURE_MAG_FILTER] || gl.NEAREST);
            gl.texParameteri(options["target"] || gl.TEXTURE_2D, gl.TEXTURE_WRAP_S,     options[gl.TEXTURE_WRAP_S]     || gl.CLAMP_TO_EDGE);
            gl.texParameteri(options["target"] || gl.TEXTURE_2D, gl.TEXTURE_WRAP_T,     options[gl.TEXTURE_WRAP_T]     || gl.CLAMP_TO_EDGE);
    ...
    function bind (sampler1i, width, height) {
        ...
        gl.activeTexture(gl['TEXTURE' + sampler1i]);
        ...
        framebuffer && gl.viewport(0, 0, width, height);
    ...
    Effect.prototype.dimensions = function () {
     ...
     Effect.prototype.activate = function (u) {
        ...
        gl.activeTexture(gl.TEXTURE0 + (typeof u === 'number' ? u : this.parameter("sampler")));
    ...
    function copy (texture_, x_, y_, width_, height_) {
        
        var gl = this._gl,
            // Re-assign arguments to variables, because the V8 optimizer does not optimize when modified directly.
            texture = texture_,
            x = x_,
            y = y_,
            width = width_,
        ...
        if( !(texture instanceof WebGLTexture))
            gl.bindTexture(gl.TEXTURE_2D, (texture = this._texture));
            
        x      = x || 0;
        y      = y || 0;
            
        width  = width  || ('__width__'  in texture ? texture.__width__  :  (width  || gl.canvas.width )),
        height = height || ('__height__' in texture ? texture.__height__ :  (height || gl.canvas.height))
    ...
    Effect.prototype.parameter = function (name, type_, arg_) {
        
        var gl         = this._gl,
            program    = this._program,
            params     = this._parameters, fn, location,
            // Assign parameters to variables so the function will be optimized by the V8 optimizer
            type       = type_,
            arg        = arg_;
        ...
        debug("set parameter (name, type, value)", name, type, arg)
    ...
    Effect.prototype.interpolate = function (name_, alpha_) {
        
        var name = name_, alpha = alpha_,
    ...
    Effect.prototype.quad = function (name, location) {
        ...
        with(this._gl) {
            ...
            bindAttribLocation(this._program, location, name);
        ...
        this.use = function () {
            ...
            return Effect.prototype.use.call(this);
        }
        
        this.destroy = function () {
            var gl = this._gl;
            return gl.deleteBuffer(buffer), Effect.prototype.destroy.call(this);
        }
        
        return this;
    }
    
    Effect.prototype.getContext = function (fn) {
        return this.use(), fn.call(this, this._gl, this._program), this;
    }
    ...
    function interpolate(a_, b_, alpha) {
        ...
        if(arguments.length > 3 && ! (arguments.length % 3) ) {
            ...
            result = [result, interpolate.apply(null, args)];
