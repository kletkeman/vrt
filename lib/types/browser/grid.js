        ...
        this.brush = brush = d3.svg.brush()
            .on("brushend", function () {
                ...
                if(!brush.empty() && (extent = brush.extent())) {
                    ...
                    debug("rectangle dimensions", extent[1]);
                    
                    extent[1][0] = (magnification2f[0] * width) / extent[1][0];
                    extent[1][1] = (magnification2f[1] * height) / extent[1][1];
                    
                    debug("calculate new zoom factors xy", extent[1]);
                    
                    extent[0][0] = (width  * (offset2f[0] * extent[1][0])) + extent[0][0];
                    extent[0][1] = (height * (offset2f[1] * extent[1][1])) + extent[0][1];
                    
                    extent[0][0] = extent[0][0] * (1. / width),
                    extent[0][1] = extent[0][1] * (1. / height);
                    
                    debug("calculate new shift xy", extent[0]);
                    
                    extent[0][2] = extent[1][2] = false;
    ...
    Grid.prototype.shift = function (dx, dy, magnification2f, incremental) {
        ...
        with (gl = (this.canvas.node().getContext("webgl")) ) { with(this.__WebGLObjects) {
            ...
            if(queue.indexOf(zoom) === -1)
                queue.push(zoom);//transition(transition_time_milliseconds));
