...
vrt.Api.DataSet.prototype.reload = function(check) {
		...
		var context = this, event = new Event("load");
		...
		vrt.data(this.id, function load_data_callback(err, data, eof) {
			...
			for(var k in data) {
                if(context._queue && !context._queue.length && eof)
                    context.event = new Event("load:complete");
...
vrt.Api.DataSet.prototype.onReceive = function(data, x, y) {
		...
		if(!this.event || this.event.type !== 'load') {
			if(this.reload(2))
        ...
        this.event = $.extend(new Event('receive'), {x: x, y: y, data: data});
