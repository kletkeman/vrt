$.extend(vrt.Api.DataSet.prototype, {
	...
	onResize : function(event, w, h) {
        ...
        this.event = $.extend(new Event('onResize'), {w: w, h: h});
  	...
  	onReceive : function(data, x, y) {
		...
		if(arguments.callee.caller === vrt.store.receive)
			if(this.reload(2))
				return this._queue.push(arguments);
			else if(!this.reload(1))
				return;
        
        this.event = $.extend(new Event('onReceive'), {x: x, y: y, data: data});
  	...
  	onError : function(err) {
        this.event = $.extend(new Event('onError'), {error: err});
        vrt.log.error(err);
  	},
  	
  	onSave : function(data) {
        var context = this;
        this.event = $.extend(new Event('onSave'), {data: data});
		$.extend(this, data);
		this.async(this.resize, undefined, function() {
            context.event = null;
        });  
  	},
    ...
    onDestroy: function() {
  	...
  	update : function() {
        var event = this.event, context = this;
        setTimeout(function() {
            context.event = event;
            context.async(context.draw, undefined, function() {
                context.event = null;
            });
        }, 0);
	},
