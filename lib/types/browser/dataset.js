$.extend(vrt.Api.DataSet.prototype, {
	...
	show: function (container) {
		...
		if(this.bufferSize && !this.data.__loaded__) {

			var context = this;

			d3.select(this.element).style('opacity', .5);
			...
			vrt.data(this.id, function load_data_callback(err, data, eof) {
				...
				if(eof) {
					...
					context.data.__loaded__ = eof;

					d3.select(context.element).style('opacity', 1) && context.update();
	...
	onResize : function(event, w, h) {
  	...
  	onReceive : function(data, x, y) {
		...
		if(!this.data.__loaded__ && arguments.callee.caller.name !== 'load_data_callback')
			return;
