...
vrt.Api.DataSet.prototype.render = function() {
    ...
    var context = this, 
        ...
        fname,        
        events, 
        event = this.event, events = (this.__render_event_queues = this.__render_event_queues || []),
    ...
    function render() {
        ...
        while(e = events.shift()) {
            ...
            (context.event = e.event), e.event.count++, context[e.function].call(context), (context.event = null);
    ...
    if(events.push({'event': event, 'callback': callback, 'function': fname}) > 1)
        return 0;
    
    return (function processEventQueue () {
        
        if(events.length)
            context.step < (1000 / 60) ? requestAnimFrame(processEventQueue) : setTimeout(processEventQueue, context.step);
...
vrt.Api.DataSet.prototype.onReceive = function(data, x, y) {
		...
		return this.render(function truncate () {
