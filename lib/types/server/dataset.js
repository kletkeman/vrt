define(['lib/api', 'lib/types/base/dataset', 'guid'], function(vrt, DataSet) {
    ...
    DataSet.prototype.onCreate = function(constructor) {
        ...
        if (constructor === DataSet) {
            ...
            vrt.ipc.on(this.id+'$onDelete', function(data, packet) {
                context.save(data, false);
                vrt.log.debug("DataSet#onDelete() Received IPC event", packet, data);
            }),
    ...
    DataSet.prototype.onDestroy = function(data, packet) {
        ...
        vrt.log.debug("DataSet#onDestroy()", packet&&packet.event ? "Received IPC event" : '', packet||'', "Destroyed object", JSON.stringify(this));
