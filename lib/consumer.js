define(['module', 'path', 'jquery', '../package.json', 'msgpack', 'net', 'cluster', 'fs', 'optimist', 'child_process', 'lib/api'], 
function (module, path, $, package, msgpack, net, cluster, fs, optimist, child_process, vrt) {
    ...
    function Consumer(options) {
        ...
        options = $.extend({}, config.consumer, options);
    ...
    Consumer.prototype.start = function() {
        ...
        function producers() {
            fs.readdirSync(dir = __dirname + '/../producers/').sort().forEach(function(p) {
                var child;
                context.workers.push(child = child_process.fork('lib/consumer.loader.js', ['--filename='+(dir + p)]));
