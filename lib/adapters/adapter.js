...
define(['jquery', 'lib/data3', 'js/viewport', 'eventemitter'], function ($, Data3, ViewPort, EventEmitter) {
    ...
    function load_event_handler (context, callback) {
        ...
        return function handler (file) {
            ...
            if(file.constructor.name === "File" || (this.status === 200 && (file = this.response)))
                return load.call(context, file, callback);
            
            else if(typeof callback === "function")
                callback.apply(context, args.length ? args : 'status' in this && this.status !== 200 ? [new Error(this.statusText)] : [this.error || (context.clear(), null), this.result]);
    ...
    function load (url, callback) {
        ...
        if( url && typeof url === "object" ) {
            ...
            if( url.constructor.name === "File" || url.constructor.name === "Blob" ) {
                ...
                reader.onerror = load_event_handler(this, callback);
                ...
                this.options.url = typeof chrome === "object" && chrome.fileSystem ? chrome.fileSystem.retainEntry(url) : "";
